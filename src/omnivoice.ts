 ...
 */
const DEFAULT_REF_AUDIO = join(
  dirname(fileURLToPath(import.meta.url)),
  '..',
  'assets',
  'kristin-reference.wav',
);
...
function defaultPython(): string {
  if (process.env.EBL_OMNIVOICE_PYTHON) return process.env.EBL_OMNIVOICE_PYTHON;
...
function defaultHelper(): string {
  if (process.env.EBL_OMNIVOICE_HELPER) return process.env.EBL_OMNIVOICE_HELPER;
...
// ---------------------------------------------------------------------------
// eBL normalized units → ARPABET
// ---------------------------------------------------------------------------
//
// ARPABET is a coarser inventory than the IPA Piper consumes. The mapping below
// stays on the ARPABET symbols an English OmniVoice was trained on and records
// every place that is lossier than the Piper path, so explain()/--detail can show
// it. Vowel LENGTH is realized downstream (dsp WSOLA), so here a long vowel only
// shifts vowel QUALITY (AH→AA etc.) — the honest, documented ARPABET proxy.

interface ArpaVowel {
  short: string;
  long: string;
}
const ARPA_VOWEL: Record<string, ArpaVowel> = {
  a: { short: 'AH', long: 'AA' },
  e: { short: 'EH', long: 'EY' },
  i: { short: 'IH', long: 'IY' },
  u: { short: 'UH', long: 'UW' },
...
const ARPA_CONS: Record<string, ArpaCons> = {
  b: { arpa: ['B'] },
  p: { arpa: ['P'] },
  d: { arpa: ['D'] },
  t: { arpa: ['T'] },
 ...
 */
export function toArpabet(units: PhonemeUnit[]): ArpabetResult {
  const tokens: string[] = [];
  const notes: string[] = [];
...
export function omnivoiceAvailable(opts: OmniVoiceOptions = {}): boolean {
 ...
 */
export async function renderOmniVoiceUnits(
  units: PhonemeUnit[],
  opts: OmniVoiceOptions = {},
): Promise<{ pcm: Pcm; arpabet: string; notes: string[] }> {
  const python = opts.python ?? defaultPython();
  const helper = opts.helper ?? defaultHelper();
  ...
  if (!existsSync(helper))
    throw new OmniVoiceError(
      `OmniVoice helper not found: ${helper}\n` +
        `Set EBL_OMNIVOICE_HELPER to the path of scripts/omnivoice_render.py.`,
    );
  if (!existsSync(refAudio))
    throw new OmniVoiceError(
      `OmniVoice reference audio not found: ${refAudio}\n` +
  ...
  try {
    ...
    if (res.error)
      throw new OmniVoiceError(
        `failed to launch OmniVoice python (${python}): ${res.error.message}\n` +
          `Set EBL_OMNIVOICE_PYTHON to an interpreter that can 'import omnivoice'.`,
      );
    if (res.status !== 0)
      throw new OmniVoiceError(
        `omnivoice_render.py exited ${res.status}.\n${(res.stderr || '').trim()}`,
      );
    if (!existsSync(out))
