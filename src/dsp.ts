...
export function parseWav(buf: Uint8Array): Pcm {
  ...
  for (let i = 0; i < frames; i++) {
    ...
    for (let c = 0; c < channels; c++) {
      ...
      let s: number;
      if (bits === 16) s = dv.getInt16(p, true);
      else s = (buf[p] - 128) << 8; // 8-bit unsigned -> signed 16
      acc += s;
    }
...
export function trimSilence(pcm: Pcm, thresh = 0.02, padMs = 5): Pcm {
  const s = pcm.samples;
  ...
  if (a >= b) return pcm; // all silence — leave as-is
 ...
 */
export function wsolaStretch(pcm: Pcm, factor: number): Pcm {
  if (Math.abs(factor - 1) < 1e-3 || pcm.samples.length < 2048) return pcm;
  ...
  while (yPos < targetLen) {
    ...
    if (prevTail) {
      ...
      for (let delta = -tol; delta <= tol; delta++) {
        ...
        if (s0 < 0 || s0 + overlap >= x.length) continue;
        let dot = 0;
        for (let k = 0; k < overlap; k++) dot += x[s0 + k] * prevTail[k];
        if (dot > best) {
          best = dot;
          bestDelta = delta;
        }
 ...
 */
export function findVowelNuclei(
  pcm: Pcm,
  opts: {
    frameMs?: number;
    minMs?: number;
  ...
  for (let f = 0; f < nFrames; f++) {
    ...
    let acc = 0;
    for (let k = a; k <= b; k++) acc += rms[k];
    sm[f] = acc / (b - a + 1);
  }

  let peak = 0;
  for (let f = 0; f < nFrames; f++) if (sm[f] > peak) peak = sm[f];
  if (peak <= 0) return [];
  ...
  while (f < nFrames) {
    if (sm[f] >= gate && zcr[f] <= maxZcr) {
      const a = f;
      while (f < nFrames && sm[f] >= gate && zcr[f] <= maxZcr) f++;
      runs.push({ a, b: f });
    } else f++;
  }
  ...
  for (const { a, b } of runs) {
    ...
    for (let f2 = a + 1; f2 < b - 1; f2++)
      if (sm[f2] >= sm[f2 - 1] && sm[f2] > sm[f2 + 1]) peaks.push(f2);
    if (peaks.length === 0) {
    ...
    for (let k = 0; k < kept.length; k++) {
      ...
      if (core > 0) {
        ...
        let localPeak = 0;
        for (let f2 = st; f2 < en; f2++) if (sm[f2] > localPeak) localPeak = sm[f2];
        const floor = localPeak * core;
        let cs = kept[k];
        let ce = kept[k];
        while (cs > st && sm[cs - 1] >= floor) cs--;
        while (ce < en - 1 && sm[ce + 1] >= floor) ce++;
...
export function stretchSpan(
  ...
  if (b - a < 512 || Math.abs(factor - 1) < 1e-3) return pcm;
  const head: Pcm = { sampleRate: pcm.sampleRate, samples: s.slice(0, a) };
  const mid: Pcm = { sampleRate: pcm.sampleRate, samples: s.slice(a, b) };
  const tail: Pcm = { sampleRate: pcm.sampleRate, samples: s.slice(b) };
...
export function concatCrossfade(parts: Pcm[], fadeMs = 12): Pcm {
  const clean = parts.filter((p) => p.samples.length > 0);
  if (clean.length === 0) return { sampleRate: 22050, samples: new Int16Array(0) };
