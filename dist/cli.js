    ...
    for (let i = 0; i < argv.length; i++) {
        ...
        switch (t) {
            ...
            case '-h':
                ...
                break;
            default:
                if (!a.ipa && !t.startsWith('-'))
                    a.ipa = t; // allow bare positional IPA
                else
...
async function main() {
    ...
    if (args.checkVoice) {
        if (voiceAvailable(args.voicePath)) {
            ...
            process.stdout.write(`neural voice NOT installed. Run:  ebl-tts --fetch-voice\n` +
    ...
    process.stdout.write(`wrote ${out} (${r.wav.length} bytes, ${r.durationSeconds.toFixed(3)}s, ` +
        `${r.sampleRate} Hz, engine=${r.engine}` +
        `${r.neuralEngine ? `/${r.neuralEngine}` : ''}${r.voice ? ` voice=${r.voice}` : ''})\n`);
    if (args.detail) {
        if (r.engineReason)
            process.stdout.write(`  engine reason: ${r.engineReason}\n`);
        process.stdout.write(`  phonemes: ${r.phonemes}\n`);
        process.stdout.write(`  extra-long: ${r.ultralong}${r.ultralongFactor !== 1 ? ` (${r.ultralongFactor}×)` : ''}\n`);
